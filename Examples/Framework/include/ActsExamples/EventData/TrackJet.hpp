...
class TrackJet {
 ...
 public:
  ...
  void setLabel(const ActsExamples::jetlabel jl) {
    m_label = jl;
  }
  
  ActsExamples::jetlabel getLabel() const {
    return m_label;
  }
  ...
  void print() const {
    ...
    std::cout<<"4mom=("<<m_fourMomentum(0)<<","<<
        m_fourMomentum(1)<<","<<
        m_fourMomentum(2)<<","<<
        m_fourMomentum(3)<<")"<<std::endl;
    ...
    std::cout<<"With  "<< m_trk_idxs.size()<<" associated tracks"<<std::endl;
